...
var URL = 'http://192.168.43.245';


const socket = io(URL + ':1000', {autoConnect: true});
socket.on('connect', () => {
...
export default class Admin extends Component{
    ...
    handleChange() {
        ...
        this.state.on = non;
        this.setState({on: non, time1: th.state.time1, time2: th.state.time2});
        if(this.state.on){
            socket.emit('webToServer', 'ON');
        }
        else{
            socket.emit('webToServer', 'OFF');
        }
    }
